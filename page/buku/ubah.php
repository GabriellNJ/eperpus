...
if(isset($_POST['ubah'])) {
    ...
    if(empty($foto && $judul && $kategori  && $pengarang && $penerbit && $tahun_terbit && $isbn && $jumlah && $lokasi  && $desk && $tgl_input)) {
        echo "<script>alert('Pastikan anda sudah mengisi semua formulir.');window.location='?p=buku';</script>";
    }

	$sql = $conn->query("UPDATE tb_buku SET foto = '$foto', judul_buku = '$judul', kategori = '$kategori',pengarang_buku = '$pengarang', penerbit_buku = '$penerbit', tahun_terbit = '$tahun_terbit', isbn = '$isbn', jumlah_buku = '$jumlah', lokasi = '$lokasi', desk = '$desk', tgl_input = '$tgl_input' WHERE id_buku = $id_buku") or die(mysqli_error($conn));
	if($sql) {
...
<div class="card-header mb-5">
	...
	<div class="form-group">
					...
					<br>
					<img src="foto/<?php echo $pecahSql['foto']; ?>" style=" width:70px; height:90px;" />
					<br>
					<br>
    ...
    <label for="kategori">Kategori</label>
    	<select name="kategori" id="kategori" class="form-control">
    		...
    		<option value="Novel" <?php if($pecahSql['kategori'] == 'Novel'){echo "selected";} ?> >Novel</option>
    		<option value="Komik" <?php if($pecahSql['kategori'] == 'Komik'){echo "selected";} ?> >Komik</option>
    		<option value="majalah" <?php if($pecahSql['kategori'] == 'Majalah'){echo "selected";} ?> >Majalah</option>
    		<option value="Biografi" <?php if($pecahSql['kategori'] == 'Biografi'){echo "selected";} ?> >Biografi</option>
    		<option value="Kamus" <?php if($pecahSql['kategori'] == 'Kamus'){echo "selected";} ?> >Kamus</option>
    		<option value="Naskah" <?php if($pecahSql['kategori'] == 'Naskah'){echo "selected";} ?> >Naskah</option>
    	</select>
    </div>
    <div class="form-group">
	...
	<div class="form-group">
        <label class="small mb-1" for="desk">Deskrisi</label>
        <input class="form-control" id="desk" name="desk" type="text" value="<?= $pecahSql['desk']; ?>" placeholder="Masukan deskripsi buku"/>
    </div>
    <div class="form-group">
